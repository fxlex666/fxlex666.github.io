	...
	function main() {
		...
		function iceCream(type){
			...
			this.show = function(){
				...
				if(this.y>=game.iceY-((this.num-1)*c_height*0.094)){
					this.y = game.iceY-((this.num-1)*c_height*0.094);
					this.animate++;
 ...
 * 处理游戏线束方法
 */
function iceGameOver(score) {
    $("#playScore").val(score);
	var phbScore = 100-timea;
	if(phbScore<0) phbScore=0;
	if(timea<22&&score==0) phbScore=0;
    var host = $("#sitePath").val();
    var sendAwardWay = parseInt($("#sendAwardWay").val());

    /* 更新当前玩家分数 */
    $.post(host + "game/updateScore", { score:phbScore }, function(response){}, "json");

    /* 刷新活动信息 */
    refreshActivityInfo();

    var remainDrawTime = parseInt($("#remainDrawTime").val());
    var drawScoreLimit = parseInt($("#drawScoreLimit").val());
    if (remainDrawTime < 1 || drawScoreLimit>score) {
        $(".drawBtn").hide();
    } else if (sendAwardWay == 3) {
        $(".drawBtn").hide();
    } else {
        $(".drawBtn").show();
        $(".again_btn").hide();
...
/**弹窗动画**/
 function resultPlay3(score){
	var drawScoreLimit = parseInt($("#drawScoreLimit").val());
	$('.result_quan').addClass('fangda');
	setTimeout(function(){
		$('.over_tit').show();
		$('.over_tit').addClass('suoxiao');
		var sendAwardWay = parseInt($("#sendAwardWay").val());
		if(drawScoreLimit<=score && sendAwardWay != 3){
			
			$('.drawBtn').show();
			
			$('.drawBtn').addClass('btnDh');
		}else{
			$('.again_btn').show();
			
			$('.again_btn').addClass('btnDh');
		}

		
		setTimeout(function(){
			$('.backIndex_btn').show();
			$('.backIndex_btn').addClass('btnDh');
			setTimeout(function(){
				$('.game_over .share_btn').show();
				$('.win_wap .share_btn,.fail_wap .share_btn').show();
				$('.win_img_bot').addClass('bounceIn');
				$('.game_over .share_btn').addClass('btnDh');
				$('.win_wap .share_btn,.fail_wap .share_btn').addClass('btnDh');
			},200);
		},200);
	},400);
} 

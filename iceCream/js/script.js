...
$(function() {
	...
	var beishu = 750/1217*iheight/480*2;
...
function Screen() {
	//ipad适配 横屏
	if(window.innerWidth>=800&&window.innerHeight<window.innerWidth){
		var ipadw = 750/1217*window.innerHeight;
		//更新字体大小
		var docEl = document.documentElement
		docEl.style.fontSize = 10 * (ipadw / 320) + 'px';
		//更新页面大小
		$(".index,.game_rule,.ready,.main,.click_btn,.demoTop_tips,.indexBox,.game_over,.phb_wap,.rule_wap,.jp_wap,.win_wap,#game-box").css({"right":'0','width':`${ipadw}px`,"margin":'0 auto'})
		$("#game-box").css("height","auto");
		$(".showAwardBox,.showAwardBox div ul,.footerAd").css({"height":'36px',"line-height":'36px'})
	}
	//ipad适配 竖屏
	if(window.innerWidth>=800&&window.innerHeight>window.innerWidth){
		
		$("#game-box").css({"height":`${800/480*window.innerWidth-200 }px`,"width":'100%'});
	}
}